...
Linux 用户经常会惊讶地发现在 FreeBSD 中 Bash 不是默认的shell。事实上，默认安装中不包括 Bash。相反，默认用户shell是与 Bourne shell-兼容的 sh(1)。在 FreeBSD 13 及更早版本中默认用户 shell 是 tcsh(1)，在 FreeBSD 14 及之后版本中是 sh(1)。sh(1)与 Bash 非常相似，但功能集较小。通常为 sh(1)编写的脚本将在 Bash 中运行，反之则并非总是成立。

然而，在 FreeBSD 软件包和定制集合中可安装 Bash 和其他 shells。

安装另一个 shell 后，使用 chsh(1) 来更改用户的默认 shell。建议保持 root 用户的默认 shell 不变，因为 shells 不包含在基础分发中，安装到 /usr/local/bin。如果出现问题，/usr/local/bin 所在的文件系统可能未挂载。在这种情况下， root 将无法访问其默认 shell，导致 root 无法登录并解决问题。

## 3. FreeBSD 软件包和 Ports：在 FreeBSD 中添加软件

FreeBSD 提供了两种安装应用程序的方法：二进制软件包和编译ports。每种方法都有其自身的好处：
...
Ports集合，有时被称为ports树，可以使用 Git 安装到/usr/ports。在 FreeBSD 手册的 4.5.1 节中可以找到安装Ports集合的详细说明。

要编译port，请切换到port的目录并开始构建过程。以下示例从Ports集合安装 Apache 2.4：
...
使用ports安装软件的好处是能够定制安装选项。此示例指定了也应安装 mod_ldap 模块：
