...
在编写发布周期时间表时，需要考虑许多事项，特别是目标日期依赖于预定义的里程碑，这些里程碑具有依赖性。例如，Ports发布标记源自最后一次活跃季度分支时的活跃季度分支。这在某种程度上定义了使用哪个季度分支，发布标记可以何时发生，以及最终ports树的哪个修订版本用于最终 RELEASE 构建。
...
本节介绍了 FreeBSD 发布周期的一般程序，从主分支开始。
...
本节介绍了 FreeBSD 发布周期的一般流程，从已建立的 stable/分支开始。
...
本节介绍了将 FreeBSD 开发快照和发布版发布到项目镜像的过程。
...
此部分介绍了在发布达到生命周期结束（End-of-Life）时需要更新的与网站相关的文件。
